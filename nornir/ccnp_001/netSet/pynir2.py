...
from nornir_netmiko.plugins.tasks import netmiko_send_config
...
def load_switchport(task):
    b = task.run(task=template_file,name="Building Switchport Configuration",template="switchport.j2", path="./templates")
    ...
    task.run(task=netmiko_send_config, name="Pushing Switchport Commands", config_commands=switchport_send)
